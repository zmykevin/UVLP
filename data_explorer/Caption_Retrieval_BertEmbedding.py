...
if __name__ == "__main__":
	...
	#Load the npy file
	caption_embedding = np.load("/data/home/zmykevin/vinvl_data/CC/CC_caption_BertEmbedding_sorted.npy")
	...
	for i in tqdm(range(0, object_embedding.shape[0], batch_size)):
	    ...
	    if int((i+valid_batch_size)/save_checkpoint) > checkpoint_index:
	        ...
	        save_json(closest_captions, os.path.join(output_path, "captions_retrieved_bertembedding_top10_sorted_backup.json")) 
	        save_csv(visualization, os.path.join(output_path, "captions_retrieved_bertembedding_top10_sorted_backup.csv"))
	        save_json(error_ids, os.path.join(output_path, "captions_retrieved_bertembedding_top10_sorted_error_ids_backup.json"))
	        checkpoint_index +=1

	#save the json file
	print("finish retrieving and start to save the json and checkpoints. ")
	save_json(closest_captions, os.path.join(output_path, "captions_retrieved_bertembedding_top10_sorted.json"))   
	#save the csv file
	save_csv(visualization, os.path.join(output_path, "captions_retrieved_bertembedding_top10_sorted.csv"))
	save_json(error_ids, os.path.join(output_path, "captions_retrieved_bertembedding_top10_error_ids.json"))