...
class TrainerTrainingLoopMixin(ABC):
    ...
    def _forward(self, batch: Dict[str, Tensor]) -> Dict[str, Any]:
        ...
        #print("print current  number: {}".format(self.current_epoch))
        #print("training.epoch_aware is: {}".format(getattr(self.training_config, "epoch_aware", None)))
        prepared_batch = to_device(batch, self.device)
        self.profile("Batch prepare time")
        # Arguments should be a dict at this point

        with torch.cuda.amp.autocast(enabled=self.training_config.fp16):
            if getattr(self.training_config, "epoch_aware", False):
                model_output = self.model(prepared_batch, current_epoch=self.current_epoch)
            else: 
