...
#################Added by Mingyang#######################
@registry.register_processor("masked_region_OMVM")
class MaskedRegionOMVMProcessor(BaseProcessor):
    """
    ...
    def __call__(self, item, OMVM, MRTM_nps):
        image_labels = []

        # print(len(OMVM))
        # print(item.shape[0])
        #assert len(OMVM) == item.shape[0]
        MRTM_labels = []
        for i in range(item.shape[0]):
            prob = random.random()
            # mask token with 15% probability * the OMVM probability
            if i < len(OMVM):
                mask_threshold = self.mask_prob*OMVM[i]
                #get the mrtm_label

                if MRTM_nps is not None and len(MRTM_nps) > 0:
                    if MRTM_nps[i] is not None and len(MRTM_nps[i]) > 0:
                        try:
                            current_mrtm_token = random.choice(self.tokenize(MRTM_nps[i]))
                            current_mrtm_label = self._convert_tokens_to_ids(current_mrtm_token)

                        except:
                            current_mrtm_label = -1
                    else:
                        current_mrtm_label = -1
                else:
                    current_mrtm_label = -1
            else:
                mask_threshold = self.mask_prob
                current_mrtm_label = -1
            ...
            if prob < mask_threshold:
                ...
                MRTM_labels.append(current_mrtm_label)
            else:
                # no masking token (will be ignored by loss function later)
                image_labels.append(-1)
                MRTM_labels.append(-1)
        if MRTM_nps is None:
            return torch.tensor(image_labels, dtype=torch.long),None
        else:
            return torch.tensor(image_labels, dtype=torch.long), torch.tensor(MRTM_labels, dtype=torch.long)
#########################################################
