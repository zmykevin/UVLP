...
################################Added by Mingyang ##############################
@registry.register_processor("masked_token_image_phrase")
class MaskedTokenImagePhraseProcessor(BaseProcessor):
    _CLS_TOKEN = "[CLS]"
    _SEP_TOKEN = "[SEP]"
    _MASK_TOKEN = "[MASK]"
    _PAD_TOKEN_ID = 0
    ...
    def _random_word(
        self, tokens: List[str], tokens_nps: List[str], probability: float = 0.15
    ) -> Tuple[List[str], List[int]]:
        labels = []
        for idx, token in enumerate(tokens):
            prob = random.random()
            #check if this token belongs to the noun phrases
            #print(tokens_nps)
            if prob < probability and ((token in tokens_nps) or not tokens_nps):
                prob /= probability
    ...
    def _convert_to_indices(
        ...
        """
        tokens_a, label_a = self._random_word(tokens_a, tokens_a_np, probability=probability)
    ...
    def __call__(self, item: Dict[str, Any]):
        text_a = item["text_a"]
        text_a_np = item["text_a_np"]
        text_b = item.get("text_b", None)

        tokens_a = self.tokenize(text_a)
        tokens_a_np = []
        for c_np in text_a_np:
            if len(c_np) > 0:
                c_np_tokens = self.tokenize(c_np)
                tokens_a_np += c_np_tokens
        tokens_b = None
        ...
        output = self._convert_to_indices(
            tokens_a, tokens_a_np, tokens_b, probability=self._probability
        )
        output["is_correct"] = torch.tensor(
            item.get("is_correct", True), dtype=torch.long
        )
        #Add the target tensor by Mingyang
        output["targets"] = torch.zeros(2, dtype=torch.float) 
        if item.get("is_correct", True):
            output["targets"][1] = 1.0
        else:
            output["targets"][0] = 1.0

        return output



@registry.register_processor("masked_token_image_tag")
class MaskedTokenImageTagProcessor(BaseProcessor):
    _CLS_TOKEN = "[CLS]"
    _SEP_TOKEN = "[SEP]"
    _MASK_TOKEN = "[MASK]"
    _PAD_TOKEN = "[PAD]"
    ...
    def _truncate_seq_pair(
        self, tokens_a: List[str], tokens_b: List[str], visual_tag_boxes: List[Tensor], max_length: int
    ):
        """Truncates a sequence pair in place to the maximum length."""
    ...
    def _convert_to_indices(
        ...
        #Append the tag position for the CLS and SEP TOken
        visual_tag_boxes = [torch.zeros_like(visual_tag_boxes[0])] + visual_tag_boxes + [torch.zeros_like(visual_tag_boxes[0])]
        ...
        while len(input_ids) < self._max_seq_length:
            ...
            segment_ids.append(0)
            lm_label_ids.append(-1)
            #udpate the tokens_a_boxes
            visual_tag_boxes.append(torch.zeros_like(visual_tag_boxes[0]))
        
        #print(len(visual_tag_boxes)==self._max_seq_length)
        assert len(input_ids) == self._max_seq_length
        assert len(input_mask) == self._max_seq_length
        assert len(segment_ids) == self._max_seq_length
        assert len(lm_label_ids) == self._max_seq_length
        assert len(visual_tag_boxes) == self._max_seq_length
        input_ids = torch.tensor(input_ids, dtype=torch.long)
        input_mask = torch.tensor(input_mask, dtype=torch.long)
        segment_ids = torch.tensor(segment_ids, dtype=torch.long)
        lm_label_ids = torch.tensor(lm_label_ids, dtype=torch.long)

        #Get the visual tag boxes
        visual_tag_boxes = torch.cat([x.unsqueeze(0) for x in visual_tag_boxes], dim=0)
        # print(visual_tag_boxes.size())
        return {
            "input_ids": input_ids,
            "input_mask": input_mask,
            "segment_ids": segment_ids,
            "lm_label_ids": lm_label_ids,
            "tokens": tokens,
            "visual_tag_boxes": visual_tag_boxes
        }

    def __call__(self, item: Dict[str, Any]):
        text_a = item["text_a"]
        #Added by mingyang
        text_a_bbox = item["text_a_bbox"]
        #assert len(text_a.split()) == text_a_bbox.size(0)
        #################################################
        text_b = item.get("text_b", None)
        
        tokens_a = []
        visual_tag_boxes = []
        for i, object_tag in enumerate(text_a):
            sub_tokens = self.tokenize(object_tag)
            #append sub_tokens to tokens_a
            tokens_a.extend(sub_tokens)
            current_visual_box = text_a_bbox[i]
            visual_tag_boxes.extend([current_visual_box]*len(sub_tokens))
        #print(visual_tag_boxes[0].size())
        tokens_b = None
        # tokens_a = self.tokenize(text_a)
        # tokens_b = None
        ...
        self._truncate_seq_pair(tokens_a, tokens_b, visual_tag_boxes, self._max_seq_length)
        output = self._convert_to_indices(
            tokens_a, visual_tag_boxes, tokens_b, probability=self._probability
        )
        output["is_correct"] = torch.tensor(
            item.get("is_correct", True), dtype=torch.long
        )
        #Add the targets, this evaluation here is not important(Ignore)
        output["targets"] = torch.zeros(2, dtype=torch.float) 
        if item.get("is_correct", True):
            output["targets"][1] = 1.0
        else:
...
@registry.register_processor("masked_token_image")
class MaskedTokenImageProcessor(BaseProcessor):
    _CLS_TOKEN = "[CLS]"
    _SEP_TOKEN = "[SEP]"
    _MASK_TOKEN = "[MASK]"
    _PAD_TOKEN = "[PAD]"
    ...
    def _random_word(
        ...
        new_tokens = []
        for idx, token in enumerate(tokens):
            new_tokens.append(self._PAD_TOKEN)
            labels.append(-1)
