...
class RefCOCOAccuracy(BaseMetric):
    ...
    **Key:** ``accuracy``
    ...
    def __init__(self, score_key="scores", target_key="targets_list", topk=1):
        super().__init__("refcoco_accuracy")
        self.required_params = ["scores", "targets", "targets_list"]
    ...
    def calculate(self, sample_list, model_output, *args, **kwargs):
        ...
        # print(expected.size())
        # print(output.size())
        # print(expected==output.unsqueeze(1))
        correct = (expected == output.unsqueeze(1)).sum().float()
...
class Accuracy(BaseMetric):
    """Metric for calculating accuracy.
    **Key:** ``refcoco_accuracy``
    Prediction for multiple ground-truth
    """
