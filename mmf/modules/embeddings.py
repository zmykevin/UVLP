...
class BertVisioLinguisticImageTagEmbeddings(BertEmbeddings):
    def __init__(self, config, *args, **kwargs):
        ...
        if self.config.use_image_position_embedding:
            # print("Using image positions")
            self.visn_layer_norm = nn.LayerNorm(config.hidden_size, eps=1e-12)

            #Box Position Encoding
            self.box_fc =nn.Linear(config.visual_pos_dim, config.hidden_size)
            self.box_layer_norm =  nn.LayerNorm(config.hidden_size, eps=1e-12)

            #Tag Layer
            self.tag_layer_norm = nn.LayerNorm(config.hidden_size, eps=1e-12)
    ...
    def encode_tag(
        self, input_ids: Tensor, token_type_ids: Optional[Tensor] = None, visual_tag_box: Optional[Tensor] = None,
    ) -> Tensor:
        #print(visual_tag_box.size())
        tag_embeddings = self.word_embeddings(input_ids)
        position_embeddings = self.box_fc(visual_tag_box)
        position_embeddings = self.box_layer_norm(position_embeddings)
        tag_embeddings = self.tag_layer_norm(tag_embeddings)
        embeddings = tag_embeddings+ position_embeddings
    ...
    def encode_image_box(
        ...
        visual_embeddings = self.projection(visual_embeddings)
        visual_embeddings = self.visn_layer_norm(visual_embeddings)
        position_embeddings_visual  = self.box_fc(box)
        position_embeddings_visual  = self.box_layer_norm(position_embeddings_visual )
    ...
    def forward(
        ...
        """
        if visual_tag_box is not None:
            # print("Using image Tags.")
            # text embeddings
            text_embeddings = self.encode_tag(input_ids, token_type_ids=token_type_ids, visual_tag_box=visual_tag_box)

            # visual embeddings
            if visual_embeddings is not None and visual_embeddings_type is not None:
                v_embeddings = self.encode_image_box(
                    visual_embeddings,
