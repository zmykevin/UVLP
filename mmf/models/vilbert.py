...
class BertImageFeatureEmbeddings(nn.Module):
    ...
    def forward(self, image_feature: Tensor, image_location: Tensor) -> Tensor:
        ...
        #Added by Mingyang
        if img_embeddings.size()[1] != loc_embeddings.size()[1]:
            print("image_embedding size: {}".format(img_embeddings.size()))
            print("loc_embedding size: {}".format(loc_embeddings.size()))
        ####################
